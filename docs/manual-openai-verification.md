...
5. Inspect the poster for identity, exact spelling, three source-backed credential rows, the ABOUT block, the white-left/near-black-right diagonal split, the fixed Bay AI Circle and AGI Summit logos, the fixed “THE WORLD’S LARGEST AI SUMMIT” headline, the neon event badge, a visibly faded Palace of Fine Arts behind the portrait, the correct event footer, no Yuandong Tian remnants for unrelated speakers, and no clipping. Confirm the caption remains unchanged in structure and contains only source-backed facts and exact configured campaign values.
