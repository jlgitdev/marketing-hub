...
import { ConnectionBadge, PageState, formatDate, useWorkspace } from "./workspace";
...
export function DashboardClient() {
  ...
  return <div className="page">
    ...
    <section className="grid two section-block">
      ...
      <div className="panel"><div className="panel-heading"><h2>Latest campaign</h2><Link href="/runs" className="subtle-link">View all</Link></div>{latestCampaign ? <div className="run-row"><span className={`status-icon ${latestCampaign.status}`}><PenLine size={18}/></span><div><strong>{latestCampaign.name}</strong><p>{latestCampaign.status === "failed" ? "Generation failed · inspect the saved error" : latestCampaign.posts.map((post) => post.platform === "x" ? "X" : post.platform[0].toUpperCase()+post.platform.slice(1)).join(" · ")}</p><small>Updated {formatDate(latestCampaign.updatedAt)}</small></div><Link href={`/content?campaign=${latestCampaign.id}`} className="button secondary small">Open</Link></div> : <Empty icon={PenLine} title="No campaign yet" text="Transform selected guides into distinct platform drafts and a campaign graphic." href="/content"/>}</div>
    </section>
  </div>;
}
