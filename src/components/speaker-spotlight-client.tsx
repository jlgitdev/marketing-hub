      ...
      <div className="template-gallery">
        {templates.map((template, templateIndex) => {
          ...
          return <article className={`template-card ${template.selected ? "selected" : ""}`} key={template.id}>
            ...
            <div className="template-card-body"><div><h3>{template.name}</h3><p>v{template.version} · {template.aspectRatio} · {template.sourceType}</p></div><span className={`badge ${template.status === "ready" ? "success" : template.status === "failed" ? "danger" : "warning"}`}>{template.status === "analyzing" && <LoaderCircle className="spin" size={12}/>} {template.status}</span></div>
