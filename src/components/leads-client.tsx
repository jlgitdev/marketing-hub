...
function LeadCard({ lead, onPatch, onSave, onDelete }: { lead: LeadRecord; onPatch: (id: string, patch: Record<string, unknown>) => Promise<void>; onSave: (event: React.FormEvent<HTMLFormElement>, lead: LeadRecord) => Promise<void>; onDelete: () => Promise<void> }) {
  ...
  return <article className={`lead-card ${lead.selected ? "selected" : ""}`}>
    ...
    <div className="lead-main">
      <div className="lead-title"><div className="lead-score" aria-label={`Priority score ${lead.priorityScore} out of 100`}><strong>{lead.priorityScore}</strong><small>/100</small><span><i style={{ width: `${Math.max(4, lead.priorityScore)}%` }}/></span></div><div className="lead-identity"><div className="inline-labels"><span className={`badge ${lead.priorityTier === "hot" || lead.priorityTier === "strong" ? "success" : lead.priorityTier === "nurture" ? "warning" : ""}`}>{prettyLabel(lead.priorityTier)}</span><span className="type-label">{prettyLabel(lead.targetSegment)}</span><span className="badge">{prettyLabel(lead.salesMotion)}</span></div><h3>{lead.eventName || lead.organizationName}</h3>{lead.eventName && <p>Organized by {lead.eventOrganizer || lead.organizationName}</p>}<small>{[lead.organizationType, lead.city, lead.region].filter(Boolean).join(" · ")}{lead.eventStartDate ? ` · ${formatDate(lead.eventStartDate)}` : ""}</small></div><div className="lead-review"><label><span className="sr-only">Review status</span><select aria-label={`Review status for ${lead.organizationName}`} value={lead.reviewStatus} onChange={(event) => void onPatch(lead.id, { reviewStatus: event.target.value })}><option value="unreviewed">Unreviewed</option><option value="reviewed">Reviewed</option><option value="needs_review">Needs review</option><option value="rejected">Rejected</option></select></label></div></div>
      ...
      <div className="lead-snapshot"><div className="lead-next-action"><span>Recommended next move</span><p>{lead.nextBestAction || lead.recommendedAction}</p></div><div className="lead-contact"><span>Best contact path</span>{lead.contactEmail ? <p><strong>{lead.contactEmail}</strong><small>{[lead.contactName, lead.contactRole || lead.emailCategory.replaceAll("_", " ")].filter(Boolean).join(" · ")}</small></p> : lead.contactPageUrl ? <a href={lead.contactPageUrl} target="_blank" rel="noopener noreferrer">Open official contact page <ExternalLink size={13}/></a> : <p className="muted">No public contact method yet</p>}</div></div>
