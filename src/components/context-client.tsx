...
import { apiRequest, cleanDocumentTitle, ConnectionBadge, PageState, formatDate, useWorkspace } from "./workspace";
...
function cleanContextSummary(value: string) {
  const tableRows = value.split("|").map((part) => part.trim()).filter((part) => part && !/^[-: ]+$/.test(part));
  const cleaned = value.includes("|") ? tableRows.join(" · ") : value;
  return cleaned.replace(/#{1,6}\s*/g, "").replace(/\s+/g, " ").trim();
}
